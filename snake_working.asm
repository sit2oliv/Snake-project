...
################################################################################
# changes the direction based on the key input
majDirection:
...
####################################################
# prints the input numbers, logging purposes
printdd:
    li $v0, 1
    lw $a0 snakeDir
    syscall
...
updateGameStatus:
    ...
    li $t0 1
    move $ra $s0
    beq $v0 $t0 eatTheCandy
    jr $ra


# parameter $a0 - the cell's order number from the snake body
# output: $v0 $v1 - x and y of that cell
getCoord:
    li $t9 4 # size in bytes of a word

    la $t1 snakePosX # address for X
    la $t2 snakePosY # address for Y
    mult $a0 $t9 # calculate the offset
    mflo $t3 # get the offset
    add $t1 $t1 $t3 # address + offset for Y
    add $t2 $t2 $t3 # address + offset for Y
    lw $v0 0($t1) # get the value from X
    lw $v1 0($t2) # get the value from Y
    jr $ra

# parameter $a0 - the cell's order number from the snake body
# parameter $a1 $a2 - the values
putCoord:
    li $t9 4 # size in bytes of a word

    # get the address of table
    la $t1 snakePosX
    la $t2 snakePosY
    mult $a0 $t9
    mflo $t3 # get the needed offset
    add $t1 $t1 $t3 # address + offset for X
    add $t2 $t2 $t3 # address + offset for Y
    sw $a1 0($t1) # put the value in X
    sw $a2 0($t2) # put the value in Y
    jr $ra

# moves the snake according to the input key
moveSnake:
    move $s1 $ra
    lw $t0 tailleSnake
moveSnakeToRight:
...
# check if the head is on the same coords as the fruit

# everything that happens to the snake after he eats the fruit, so:
# increments the length, adds 1 piece to the body (not yet undderstood whether beginning or end)
isEaten:
    # store the return address
    move $s1 $ra
    # get the coords for the candy and the head
    lw $a0 candy
    lw $a1 candy + 4
    lw $a2 snakePosX
    lw $a3 snakePosY
    ...
    # check if the head is on the candy (same coords)
    jal areEqual
    move $ra $s1
    jr $ra

# first we erase the existing candy, then we create a new one,
# then we increase the length of the snake
eatTheCandy:
    move $s1 $ra
    jal deleteCandy
    jal showCandy
...
# erases the current candy from the screen
deleteCandy:
    move $s2 $ra
    lw $a0 colors + black
    lw $a1 candy
